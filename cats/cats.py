...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    # construct the list contains valid val
    def get_valid(valid_list):
        if len(valid_list) == 0:
            return []
        if len(valid_list) <= 1:
            if select(valid_list[0]):
                return [valid_list[0]]
            return []
        if select(valid_list[0]):
            return [valid_list[0]] + (get_valid(valid_list[1:]))
        return get_valid(valid_list[1:])
    # check valid list length
    if len(get_valid(paragraphs)) <= k:
        return ''
    return get_valid(paragraphs)[k]
    # END PROBLEM 1
...
def about(subject):
    ...
    def sentence_verifier(sentence):
        parsed_sentence = split(remove_punctuation(lower(sentence)))
        for token in subject:
            if token in parsed_sentence:
                return True
        return False
    return sentence_verifier
    # END PROBLEM 2
...
def feline_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    # case where typed and source are difference size
    length_diff = 0
    if len(typed) != len(source):
        length_diff = abs(len(typed) - len(source))
    # case where typed is identical to source
    if typed == source:
        return 0
    def need_a_name(dst, src, step):
        # special step
        # limit the recursion steps
        if step >= limit:
            # this should have 2 cases
            # 1. done finding all different char
            if dst == src:
                return step
            # 2. reach limit
            return limit + 1
        ...
        # guards 1
        if dst[0] == src[0]:
            return need_a_name(dst[1:],src[1:],step)
        # guards 2
        if not dst[0] == src[0]:
            return need_a_name(dst[1:],src[1:],step+1)
        return 0
    return length_diff + need_a_name(typed,source,0)
    # END PROBLEM 6
...
def report_progress(typed, source, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    correct = 0
    for word_t, word_s in zip(typed, source):
        if word_t == word_s:
            correct += 1
        else:
            break
